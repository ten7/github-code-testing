...
class Plugin implements PluginInterface, EventSubscriberInterface
{
    ...
    public function scaffold(Event $event): void
    {
        ...
        if (empty($config)) {
            $this->io->write('<info>ten7/code-tests: no code_testing config found, skipping scaffold.</info>');
            return;
        }
        ...
        foreach ($config as $context => $items) {
            foreach ($items as $item) {
                ...
                if (!file_exists($source)) {
                    $this->io->writeError("<warning>ten7/code-tests: scaffold file not found: $filename</warning>");
                    continue;
                }
                ...
                copy($source, $target);
                $this->io->write("<info>ten7/code-tests: scaffolded $filename</info>");
            }
        }
    }
    ...
    private function copyDirectory(string $source, string $target): void
    {
        ...
        foreach ($iterator as $item) {
            ...
            if ($item->isDir()) {
                ...
                $this->io->write('<info>ten7/code-tests: scaffolded action ' . $iterator->getSubPathname() . '</info>');
            }
        }
    }
}
